		...
		for key, value := range v {
			if IdField(key) {
				if strId, ok := value.(string); ok {
					decodedId, err := gut.IdDecode(strId)
					if err == nil {
						v[key] = decodedId
					}
				}
		...
		for key, value := range v {
			if IdField(key) {
				...
				case float64:
					...
					encodedId := gut.IdEncode(uint64Id)
					v[key] = encodedId
				case uint64:
					encodedId := gut.IdEncode(idVal)
					v[key] = encodedId
				case json.Number:
					if numValue, err := idVal.Int64(); err == nil {
						encodedId := gut.IdEncode(uint64(numValue))
						v[key] = encodedId
					}
				case nil:
				default:
					...
					if rv.Kind() == reflect.Ptr && !rv.IsNil() {
						if rv.Elem().Kind() == reflect.Uint64 {
							encodedId := gut.IdEncode(rv.Elem().Uint())
							v[key] = encodedId
						}
					}
				}
