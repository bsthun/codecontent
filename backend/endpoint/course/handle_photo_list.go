...
func (r *Handler) HandlePhotoList(c *fiber.Ctx) error {
	// * get user claims
	l := c.Locals("l").(*jwt.Token).Claims.(*common.LoginClaims)

	// * parse body
	body := new(payload.CoursePhotoListRequest)
	...
	// * return
	return c.JSON(response.Success(c, &payload.CoursePhotoListResponse{
		Items: items,
		Count: count,
	}))
}
